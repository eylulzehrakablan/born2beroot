...
- **|** : takes the output generated by the left side (getent) and passes it to the command on the right side (cut)
...
okay the changes are applied but when i looked at the output i have noticed that i haven't created the `/var/log/sudo/sudo.log` file. i was supposed to remember it while configuring the sudo policies, but no harm.
...
the reason we see groups as 'ekablan' and 'zehra' is, whenever we create a user account, system automatically create a user private group witht the exact same name for security and permission isolated. zehra and ekablan are both users and groups.
...
so we don't have to configure 'difok' for root, PAM already handles
```
